...
RUN apt-get update && apt-get install -y --no-install-recommends git ca-certificates curl libatomic1 libgomp1 libvulkan1 \
...
COPY tests ./tests
COPY launch.py ./
RUN mkdir -p /data /workspaces /home/agentvisor/.lmstudio \
    && chown agentvisor:agentvisor /data /workspaces /home/agentvisor/.lmstudio
